...
bool AccountRegistry::SaveAccount(const std::string& username, const std::string& filepath)
{
    fs::path fp = filepath;
    if (!fs::exists(fp))
    {
        Logger::GetInstance().Error("Attempting to save account to file path which does not exist");
        return false;
    }

    std::ofstream ofs (fp);
    ofs << username.size() << username << _accounts.at(username).size() << _accounts.at(username);
    return true;
}
bool AccountRegistry::DeleteAccount(const std::string& username, const std::string& filepath)
{
    (void)username;
    (void)filepath;
    return true;
}
bool AccountRegistry::LoadAccounts(const std::string& filepath)
{
    (void)filepath;
    return true;
}
